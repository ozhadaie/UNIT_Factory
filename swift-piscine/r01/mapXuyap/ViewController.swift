...
class ViewController: UIViewController, GMSMapViewDelegate {
    ...
    @IBAction func informationButtonPressed(_ sender: UIButton) {
        ...
        if origin.description != "" && destination.description != "" {
            let actionSheet = UIAlertController(title: "Information", message: "Route request:", preferredStyle: UIAlertController.Style.actionSheet)
            let alert1 = UIAlertAction(title: "From \(origin.description)", style: UIAlertAction.Style.default)  { (alertAction) ->  Void in }
            let alert2 = UIAlertAction(title: "to \(destination.description)", style: UIAlertAction.Style.default) { (alertAction) ->  Void in }
